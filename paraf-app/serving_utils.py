...
def _encode(inputs, encoder, add_eos=True, tgt_lang_id=None):
  input_ids = encoder.encode(inputs)
  if add_eos:
    input_ids.append(text_encoder.EOS_ID)
  if tgt_lang_id:
    input_ids.insert(0, tgt_lang_id)
  return input_ids
...
def predict(inputs_list, problem, request_fn):
  ...
  input_ids_list = [
      _encode(inputs, input_encoder, add_eos=problem.has_inputs, tgt_lang_id=tgt_lang_id)
      for inputs, tgt_lang_id in inputs_list
  ]
